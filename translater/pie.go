...
// pie 生成pie报表
// pie("name_field","value_field", size, "chart title")
// 第一个参数是显示的字段名称；第二个参数是值的字段名称，如果是count()表明是去重统计；第三个参数是top size；第四个参数是标题
func pie(fi *workflowast.FuncInfo) string {
	tmpl, _ := template.New("pie").Parse(`PieChart(GetRunner(), map[string]interface{}{
    "name": "{{ .Name }}",
    "value": "{{ .Value }}",
    "size": {{ .Size }},
    "title": "{{ .Title }}",
